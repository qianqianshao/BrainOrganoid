# scRNA-seq analysis for 6 SOSRSs at 3-month old with HTOs on 3.3.2021 by Qianyi
# 6 replicates sequenced in 2 samples of 1 run, each with 3 SOSRS (3 HTO tags)
# sample 1: HTO-1,2,3
# sample 2: HTO-4,5,6
...
datalists
datalists=datalists[c(1,2,3,4,6,5)]
datalists
...
dge$run2directmerge=dge$RNA_snn_res.0.2ordered
...
res=paste0("integrated_snn_res.0.",c(1))
resi=1
...
table(dge$run2directmerge,cells.ident.ordered[names(dge$indivclusters)])
table(dge$indivclusters,cells.ident.ordered[names(dge$indivclusters)])
...
res=paste0("integrated_snn_res.0.",c(1),"ordered")
i=resi=1
...
centroid1=read.table("plot/2514-AT_singlet_9clustercentroids.txt")
hvg2=VariableFeatures(dge2)
cor(centroid1[hvg2,],centroid2[hvg2,],method="sp")
...
write.table(centroid2,paste0("plot/2514-AT_singlet_CCA6_7clustercentroids.txt"),quote=F,row.names=T,col.names=T,sep="\t")
dim(centroid1)
dim(centroid2)
...
union=union[which(union %in% intersect(rownames(dge1),rownames(dge2)))]
...
write.table(markers,paste0("plot/hto_FitClassify_singlet_CCA6_",res,"_mindiff0.2_logfc2fold_1.2021.txt"),col.names=T,row.names=T,quote=F,sep="\t")
markers=FindAllMarkers(dge,only.pos=TRUE,logfc.threshold = log(1.5),min.diff.pct=0.1)
write.table(markers,paste0("plot/hto_FitClassify_singlet_CCA6_",res,"_mindiff0.1_logfc1.5fold_1.2021.txt"),col.names=T,row.names=T,quote=F,sep="\t")
...
dgeall=dge
for(label in c("label","integrated_snn_res.0.1ordered")){
plotlist=list()
